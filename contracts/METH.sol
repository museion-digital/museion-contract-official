...
error METH_Only_MSO_Market_Allowed();
...
contract METH {
  ...
  /// @notice The Museion market contract with permissions to manage lockups.
  address payable private museionMarket;

  /// @notice The Museion drop market contract with permissions to withdraw available funds.
  address payable private museionDropMarket;
   ...
   * @return Museion ETH
   */
  string public constant name = "Museion ETH";
  /**
  ...
  // Custom events
  /**
   * @notice Emitted when METH tokens are locked up by the Museion market for 24-25 hours
  ...
  /**
   * @notice Emitted when METH tokens are unlocked by the Museion market.
   ...
   * @dev This may be triggered by the user, an approved operator, or the Museion market.
  ...
  /// @dev Allows the Museion market permission to manage lockups for a user.
  modifier onlyMuseionMarket() {
    if (msg.sender != museionMarket && msg.sender != museionDropMarket) {
      revert METH_Only_MSO_Market_Allowed();
    }
    _;
  }
  
  /**
   * @notice Set immutable variables for the contract.
   * @dev Using immutable instead of constants allows us to use different values on testnet.
   * @param _museionMarket The address of the Museion NFT marketplace.
   */
  constructor(address payable _museionMarket, address payable _museionDropMarket) {
    if (!_museionMarket.isContract()) {
      revert METH_Market_Must_Be_A_Contract();
    }
    if (!_museionDropMarket.isContract()) {
      revert METH_Market_Must_Be_A_Contract();
    }
    museionMarket = _museionMarket;
    museionDropMarket = _museionDropMarket;
  }
   ...
   * These tokens are not subject to any lockup period.
   * @dev This may be used by the Museion market to credit a user's account with METH tokens.
   ...
   * @param duration lockup period in seconds
  ...
  function marketChangeLockup(
    ...
    uint256 duration
  ) external payable onlyMuseionMarket returns (uint256 expiration) {
    _marketUnlockFor(unlockFrom, unlockExpiration, unlockAmount);
    return _marketLockupFor(lockupFor, lockupAmount, duration);
  }
   ...
   * @return expiration The expiration timestamp for the METH tokens that were locked.
   */
  function marketLockupFor(address account, uint256 amount, uint256 duration)
    external
    payable
    onlyMuseionMarket
    returns (uint256 expiration)
  {
    return _marketLockupFor(account, amount, duration);
  }
   ...
   * This will revert if the tokens were previously unlocked.
   */
  function marketWithdrawFrom(address from, uint256 amount) external onlyMuseionMarket {
  ...
  function withdrawFrom(
    ...
    } else if (to == address(museionMarket)) {
      revert METH_Cannot_Withdraw_To_Market();
    } else if (to == address(museionDropMarket)) {
      revert METH_Cannot_Withdraw_To_Market();
    }
  ...
  function _marketLockupFor(address account, uint256 amount, uint256 duration) private returns (uint256 expiration) {
    ...
    unchecked {
      ...
      expiration = duration + block.timestamp;
    }
  ...
  /**
   * @notice Gets the Museion market address which has permissions to manage lockups.
   * @return market The Museion market contract address.
   */
  function getMuseionMarket() external view returns (address market) {
    market = museionMarket;
  }

  /**
   * @notice Gets the Museion drop market address which has permissions to withdraw available funds.
   * @return market The Museion drop market contract address.
   */
  function getMuseionDropMarket() external view returns (address market) {
    market = museionDropMarket;
  }
