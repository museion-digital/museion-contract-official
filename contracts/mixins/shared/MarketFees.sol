...
import "./MuseionTreasuryNode.sol";
 ...
 */
abstract contract MarketFees is MuseionTreasuryNode, MarketSharedCore, SendValueWithFallbackWithdraw {
   ...
   * uint256 private _primaryMuseionFeeBasisPoints;
   * uint256 private _secondaryMuseionFeeBasisPoints;
  ...
  uint256 private constant CREATOR_ROYALTY_DENOMINATOR = BASIS_POINTS / 500; // 5%
  /// @notice The fee collected by Museion for sales facilitated by this market contract.
  ...
  /**
   * @notice Distributes funds to museion, creator recipients, and NFT owner after a sale.
   */
    ...
    )
  {
    ...
    _sendValueWithFallbackWithdraw(getMuseionTreasury(), totalFees, SEND_VALUE_GAS_LIMIT_SINGLE_RECIPIENT);
   ...
   * @return totalFees How much will be sent to the Museion treasury and/or referrals.
    ...
    )
  {
    ...
    (totalFees, creatorRecipients, creatorShares, sellerRev, ) = _getFees(
      ...
      seller,
      price,
      // Notice: Setting this value is a breaking change for the MSOMiddleware contract.
    ...
    )
  {
    ...
    if (creatorRecipients.length != 0 || assumePrimarySale) {
      ...
      if (assumePrimarySale) {
        ...
        // Rounding favors the owner first, then creator, and museion last.
        unchecked {
